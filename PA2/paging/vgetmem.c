	...
	//kprintf("vmemlist points at %x\n", proctab[getpid()].vmemlist->mnext);
        ...
        for (q = proctab[getpid()].vmemlist, p = proctab[getpid()].vmemlist->mnext ;
			...
			kprintf("11111---Returning %x from vgetmen \n", (WORD*)p);
                        return(4096*4096 + (WORD *) (p-(BACKING_STORE_BASE + proctab[getpid()].store*BACKING_STORE_UNIT_SIZE)));
			...
			return( (WORD *) (4096*4096 + (p-(BACKING_STORE_BASE + proctab[getpid()].store*BACKING_STORE_UNIT_SIZE))));
                }
        restore(ps);
